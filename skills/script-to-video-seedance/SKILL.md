...
fl2v 模式对帧图内容比 ref2v 严苛很多。**即使帧图已经进合规库且
`compliant` 了,只要画面里能明显识别出人物(写实风格 character),
fl2v 生成时仍会 `real_person` 拒**。ref2v 用同样的图作 `first_frame_url`
反而 OK。

**实测结论:**
| 模式 | 帧图有明显人物(写实风格) | 帧图是纯环境/无人景 |
|---|---|---|
| `ref2v` | ✅ 过(即使未合规库,但建议走合规) | ✅ 过 |
| `fl2v` | ❌ `real_person` 拒(即使合规过) | ✅ 过 |

**fl2v 能用的场景:**
- 环境转场(空镜 → 空镜,比如白天→黑夜、下雨→雪)
- 物体动画(道具、logo、建筑物)
- 角色图是重度 stylized / 漫画风 / 非写实的(没实测过,谨慎)

**fl2v 不能用的场景:**
- character 特写 first+last(真人写实角色图一律拒)
- scene 级别但人物清晰的镜头(实测同样拒)

**替代方案(用 character 做转场):**
- 走 ref2v 模式:`first_frame_url: <scene 首帧>` + `reference_image_urls:
  [character, location]`,不传 last_frame_url,让 Seedance 自由演到结尾
- 或者两个 clip + 尾帧提链(Phase 4 的 continuous 模式),比 fl2v 还稳
...
| `real_person` | **fl2v 模式下帧图有明显人物**(实测:即使合规库 `compliant` 过的 character / scene 图,只要写实风格能识别出人物,fl2v 生成时仍拒)| fl2v 只能用无人纯环境图作帧;角色转场用 ref2v 模式(`first_frame` + `reference_image_urls`,不传 `last_frame_url`)或两个 clip 尾帧提链 |
